...
static int
virCgroupV2LoadDeviceProg(int mapfd)
{
                      ...
                      int perms)
{
    ...
    VIR_DEBUG("deny device %c %x:%x perms:%x key:%llx val:%x", type, major, minor, perms, key, val);
    ...
    if (virBPFLookupElem(group->unified.mapfd, &key, &val) < 0) {
